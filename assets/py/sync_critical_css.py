...
block of rules in style.css (e.g. @font-face, resets, body styles).

SETUP (one-time):

1. In style.css, wrap the rules you've duplicated into critical.css
...
in style.css if the borrowed rules aren't contiguous; all blocks
are concatenated in order.

2. In critical.css, add markers where the synced rules should land:

    /* AUTO-SYNCED FROM style.css — DO NOT EDIT BELOW — START */
    /* AUTO-SYNCED FROM style.css — END */
...
USAGE:
    ...
    python sync_critical_css.py style.css critical.css

    # Or watch for changes and auto-sync on save:
    python sync_critical_css.py style.css critical.css --watch
"""
...
DEST_START = "/* AUTO-SYNCED FROM style.css — DO NOT EDIT BELOW — START */"
DEST_END = "/* AUTO-SYNCED FROM style.css — END */"
...
def extract_wrapped_blocks(css_text: str) -> list[str]:
    ...
    style.css, in order, with the markers themselves stripped out."""
    pattern = re.compile(
    ...
    try:
        while True:
            ...
            if mtime != last_mtime:
                ...
                changed = sync(args.shared_css, args.critical_css)
                if changed:
                    print("style.css changed — critical.css re-synced.")
